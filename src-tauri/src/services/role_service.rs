...
impl RoleService {
    ...
    pub async fn list_roles(&self, tenant_id: Option<&str>) -> Result<Vec<RoleWithPermissions>, sqlx::Error> {
        #[cfg(feature = "postgres")]
    ...
    /// Get role by ID
    pub async fn get_role_by_id(&self, role_id: &str) -> Result<Option<RoleWithPermissions>, sqlx::Error> {
        #[cfg(feature = "postgres")]
    ...
    pub async fn update_role(
        ...
        if role.is_system && !is_super_admin {
            return Err(sqlx::Error::Protocol("System roles can only be modified by Super Admin".to_string()));
        }
    ...
    pub async fn delete_role(&self, role_id: &str, is_super_admin: bool, actor_id: Option<&str>, ip_address: Option<&str>) -> Result<bool, sqlx::Error> {
        ...
        #[cfg(feature = "sqlite")]
        let role_info: Option<(bool, Option<String>)> = sqlx::query_as("SELECT is_system, tenant_id FROM roles WHERE id = ?")
            .bind(role_id)
            .fetch_optional(&self.pool)
            .await?;
