...
impl StorageService {
    ...
    pub async fn register_upload(
        ...
        let mut tx = self.pool.begin().await.map_err(|e| AppError::Internal(format!("DB Error: {}", e)))?;
        ...
        let db_result = async {
            ...
            if let Some(max_gb) = limit_gb {
                ...
                let current_usage: i64 = sqlx::query_scalar("SELECT storage_usage FROM tenants WHERE id = $1 FOR UPDATE")
                    .bind(tenant_id)
                    .fetch_one(&mut *tx)
                    .await?;
                
                if (current_usage as u64) + (size as u64) > max_bytes {
                    return Err(AppError::Validation(format!("Plan limit reached. Max {} GB.", max_gb)));
                }
            }
        ...
        match db_result {
            ...
            Err(e) => {
                ...
                // Note: File on disk is NOT deleted here automatically. 
                // The caller should handle cleanup if DB fails.
                Err(e)
            }
        }
    }
    ...
    pub async fn list_tenant_files(
        ...
        #[cfg(feature = "postgres")]
        {
            ...
            // Count
            let mut count_qb: QueryBuilder<Postgres> = QueryBuilder::new("SELECT COUNT(*) FROM file_records WHERE tenant_id = ");
