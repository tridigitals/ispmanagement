...
pub async fn upload_file(
    ...
    let tid = claims
        .tenant_id
        .ok_or_else(|| "Global upload not supported yet (requires tenant context)".to_string())?;

    state
        .upload(&tid, &file_name, &content_type, &data, Some(&claims.sub))
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn list_files_admin(
    ...
    let (data, total) = state
        .list_all_files(page, per_page, search)
        .await
        .map_err(|e| e.to_string())?;
...
pub async fn list_files_tenant(
    ...
    let (data, total) = state
        .list_tenant_files(&tenant_id, page, per_page, search)
        .await
        .map_err(|e| e.to_string())?;
