...
fn has_permission(perms: &[String], resource: &str, action: &str) -> bool {
    ...
    perms.iter().any(|p| p == "*" || p == &perm || p == &wildcard)
}
...
async fn list_backups(
    ...
    let perms = state.auth_service.get_user_permissions(&claims.sub, tenant_id).await?;
    ...
    match payload.backup_type.as_str() {
        "global" => {
            if !claims.is_super_admin {
                return Err(crate::error::AppError::Forbidden("Only Super Admin can create global backups".to_string()));
            }
        ...
        "tenant" => {
            let target_id = match payload.target_id {
                ...
                None => {
                    ...
                    claims.tenant_id.clone().ok_or(crate::error::AppError::Forbidden(
            ...
            if !claims.is_super_admin {
                 // RBAC check (tenant)
                 let tenant_id = claims.tenant_id.as_ref().ok_or(crate::error::AppError::Forbidden("No tenant context found".to_string()))?;
                 let perms = state.auth_service.get_user_permissions(&claims.sub, tenant_id).await?;
                 if !has_permission(&perms, "backups", "create") {
                      return Err(crate::error::AppError::Forbidden("Missing permission backups:create".to_string()));
                 }
                 
                 // Check if claims.tenant_id matches target_id
                 if let Some(tid) = &claims.tenant_id {
                     if tid != &target_id {
                         return Err(crate::error::AppError::Forbidden("Cannot backup other tenants".to_string()));
                     }
                 } else {
                     return Err(crate::error::AppError::Forbidden("No tenant context found".to_string()));
                 }
            }

            let path = state.backup_service.create_tenant_backup(&target_id).await?;
            Ok(Json(path))
        },
        _ => Err(crate::error::AppError::Validation("Invalid backup type".to_string())),
    }
}
...
async fn download_backup(
    ...
    if !claims.is_super_admin {
        // RBAC check (tenant)
        let tenant_id = claims.tenant_id.as_ref().ok_or(crate::error::AppError::Forbidden("Tenant context missing".to_string()))?;
        let perms = state.auth_service.get_user_permissions(&claims.sub, tenant_id).await?;
        ...
        if !filename.starts_with("tenant_") {
            return Err(crate::error::AppError::Forbidden("Cannot access global backups".to_string()));
        }
        
        let expected_prefix = format!("tenant_{}_", tenant_id);
        
        if !filename.starts_with(&expected_prefix) {
            return Err(crate::error::AppError::Forbidden("Cannot download other tenant's backups".to_string()));
        }
    }

    // Read file and stream
    let file = tokio::fs::File::open(&file_path).await
        .map_err(|e| crate::error::AppError::Internal(format!("Failed to open backup file: {}", e)))?;
    ...
    Ok((
        [
            (axum::http::header::CONTENT_TYPE, axum::http::HeaderValue::from_static(content_type)),
            (
                axum::http::header::CONTENT_DISPOSITION,
                axum::http::HeaderValue::from_str(&disposition).map_err(|_| crate::error::AppError::Internal("Invalid header value".to_string()))?,
            ),
        ],
        body,
    ))
...
async fn restore_backup(
    ...
    while let Some(field) = multipart.next_field().await.map_err(|e| crate::error::AppError::Internal(e.to_string()))? {
        ...
        if name == "file" {
            ...
            tokio::fs::write(&temp_path, data).await.map_err(|e| crate::error::AppError::Internal(e.to_string()))?;
            file_saved = true;
        }
    }

    if !file_saved {
        return Err(crate::error::AppError::Validation("No file uploaded".to_string()));
    }

    // Determine if global or tenant based on claims
    let res = if claims.is_super_admin {
        state.backup_service.restore_from_zip(temp_path.clone(), None).await
    } else {
        let tenant_id = claims.tenant_id.as_ref().ok_or(crate::error::AppError::Forbidden("Tenant context missing".to_string()))?;
        let perms = state.auth_service.get_user_permissions(&claims.sub, tenant_id).await?;
        ...
        state.backup_service.restore_from_zip(temp_path.clone(), Some(tenant_id)).await
    };
...
async fn restore_local_backup(
    ...
    let res = if claims.is_super_admin {
        ...
        // Tenant admin check
        if !filename.starts_with("tenant_") {
            return Err(crate::error::AppError::Forbidden("Unauthorized backup access".to_string()));
        }
        let tenant_id = claims.tenant_id.as_ref().ok_or(crate::error::AppError::Forbidden("Tenant context missing".to_string()))?;
        let perms = state.auth_service.get_user_permissions(&claims.sub, tenant_id).await?;
        if !has_permission(&perms, "backups", "restore") {
        ...
        let expected_prefix = format!("tenant_{}_", tenant_id);
        if !filename.starts_with(&expected_prefix) {
            return Err(crate::error::AppError::Forbidden("Cannot restore other tenant's backups".to_string()));
        }
        state.backup_service.restore_local_backup(filename, Some(tenant_id)).await
    };
