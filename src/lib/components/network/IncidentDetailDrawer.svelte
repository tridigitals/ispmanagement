...
{#if open && incident}
  ...
  <aside class="drawer" aria-label={$t('common.details')}>
    <div class="drawer-head">
      <div>
        <div class="drawer-title">{$t('common.details')}</div>
        <div class="drawer-sub">{incident.title}</div>
      </div>
      <button class="icon-btn" type="button" onclick={onClose} title={$t('common.close')}>
        <Icon name="x" size={16} />
      </button>
    </div>
    ...
    <div class="drawer-body">
      ...
      <div class="detail-grid">
        <div class="drow"><span class="muted">{$t('admin.network.incidents.columns.status')}</span><span class="mono">{incident.status}</span></div>
        <div class="drow"><span class="muted">{$t('admin.network.incidents.columns.type')}</span><span class="mono">{typeLabel(incident.incident_type)}</span></div>
        <div class="drow"><span class="muted">{$t('admin.network.incidents.columns.severity')}</span><span class="mono">{severityLabel(incident.severity)}</span></div>
        <div class="drow">
          <span class="muted">{$t('admin.network.incidents.labels.auto_escalated')}</span>
          <span class="mono">
        ...
        <div class="drow"><span class="muted">{$t('admin.network.incidents.columns.seen')}</span><span class="mono">{formatDateTime(incident.last_seen_at, { timeZone: appTimezone })}</span></div>
        <div class="drow">
          <span class="muted">{$t('admin.network.incidents.drawer.email_notify')}</span>
          <span class="mono">
        ...
        <div class="drow">
          <span class="muted">{$t('admin.network.incidents.sla.title')}</span>
          <span class="mono">
        ...
        <div class="drow"><span class="muted">{$t('admin.network.incidents.drawer.assignee')}</span><span class="mono">{ownerLabel(incident.owner_user_id)}</span></div>
        <div class="drow"><span class="muted">{$t('admin.network.incidents.labels.router')}</span><span class="mono">{router?.identity || router?.name || incident.router_id}</span></div>
        <div class="drow"><span class="muted">{$t('admin.network.incidents.labels.interface')}</span><span class="mono">{incident.interface_name || '-'}</span></div>
        {#if router}
          <div class="drow"><span class="muted">{$t('admin.network.incidents.labels.host')}</span><span class="mono">{router.host}:{router.port}</span></div>
          <div class="drow"><span class="muted">{$t('admin.network.incidents.labels.latency')}</span><span class="mono">{router.latency_ms == null ? '—' : `${router.latency_ms} ms`}</span></div>
        {/if}
        {#if metric}
          <div class="drow"><span class="muted">{$t('admin.network.incidents.labels.cpu')}</span><span class="mono">{metric.cpu_load == null ? '—' : `${metric.cpu_load}%`}</span></div>
          <div class="drow"><span class="muted">{$t('admin.network.incidents.labels.rx_tx')}</span><span class="mono">{formatBps(metric.rx_bps)} / {formatBps(metric.tx_bps)}</span></div>
          <div class="drow"><span class="muted">{$t('admin.network.incidents.labels.memory_use')}</span><span class="mono">{memoryUsePct(metric.total_memory_bytes, metric.free_memory_bytes) == null ? '—' : `${memoryUsePct(metric.total_memory_bytes, metric.free_memory_bytes)}%`}</span></div>
        {/if}
      </div>
      ...
      <div class="impact-card">
        <div class="impact-head">
          <div class="impact-title">
            {$t('admin.network.incidents.impact.title')}
          </div>
        ...
        {:else if impactedCustomers.length === 0}
          <div class="muted">
            {$t('admin.network.incidents.impact.empty')}
          </div>
        {:else}
              ...
              <div class="impact-item">
                ...
                <div class="impact-tags">
                  {#if item.impacted_via_node}
                    <span class="impact-tag">{$t('admin.network.incidents.labels.via_node')}</span>
                  {/if}
                  {#if item.impacted_via_link}
                    <span class="impact-tag">{$t('admin.network.incidents.labels.via_link')}</span>
                  {/if}
                </div>
              </div>
            {/each}
      ...
      <div class="detail-edit">
        <div class="field">
          <label for="incident-owner">{$t('admin.network.incidents.drawer.assignee')}</label>
          {#if canManage}
            <select
              id="incident-owner"
              class="input"
        ...
        <div class="field">
          <label for="incident-notes">{$t('admin.network.incidents.drawer.notes')}</label>
          {#if canManage}
            <textarea
              id="incident-notes"
              class="textarea"
              ...
              placeholder={$t('admin.network.incidents.drawer.notes_placeholder')}
            ></textarea>
          {:else}
        ...
        {#if canManage}
          ...
          <div class="save-row">
            <button class="btn ghost" type="button" onclick={onOpenNetworkSettings}>
              ...
              {$t('admin.network.incidents.drawer.open_network_settings')}
            </button>
          </div>
        {/if}
      </div>
