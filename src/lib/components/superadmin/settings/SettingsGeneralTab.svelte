<script lang="ts">
  ...
  let timezoneOptions: string[] = fallbackTimezones;
  ...
  onMount(() => {
    try {
      ...
      if (typeof supportedValuesOf === 'function') {
        timezoneOptions = supportedValuesOf('timeZone');
      }
    } catch {
      timezoneOptions = fallbackTimezones;
    }

    if (appTimezone && !timezoneOptions.includes(appTimezone)) {
      timezoneOptions = [appTimezone, ...timezoneOptions];
    }
  });
