...
<article class="invoice-doc" class:embed aria-label={$t('components.invoice_print.preview')}>
  <!-- Header --------------------------------------------------------------- -->
  <header class="doc-head">
    ...
    <div class="doc-title">
      <h1>{$t('components.invoice_print.invoice')}</h1>
  ...
  <section class="doc-parties">
    <div class="party">
      <span class="party-label">{$t('components.invoice_print.bill_to')}</span>
    ...
    <div class="party party-meta">
      <div class="meta-row">
        <span>{$t('components.invoice_print.issue_date')}</span>
        <strong>{fmtDate(invoice?.created_at)}</strong>
      </div>
      <div class="meta-row">
        <span>{$t('components.invoice_print.due_date')}</span>
        <strong>{fmtDate(invoice?.due_date)}</strong>
      </div>
      {#if invoice?.paid_at}
        <div class="meta-row">
          <span>{$t('components.invoice_print.paid_at')}</span>
          <strong>{fmtDate(invoice.paid_at)}</strong>
        </div>
      {/if}
      {#if subscription?.billing_cycle}
        <div class="meta-row">
          <span>{$t('components.invoice_print.billing_cycle')}</span>
    ...
    <table class="item-table">
      <thead>
        <tr>
          ...
          <th class="col-price">{$t('components.invoice_print.unit_price')}</th>
          <th class="col-total">{$t('common.amount')}</th>
        </tr>
      </thead>
      <tbody>
        {#each items as item}
  ...
  <section class="doc-totals">
    ...
    <div class="totals-block">
      <div class="totals-row">
        <span>{$t('components.invoice_print.subtotal')}</span>
  ...
  {#if bankAccounts.length > 0 && invoice?.status !== 'paid'}
    <section class="doc-payment">
      <h3>{$t('components.invoice_print.payment_instructions')}</h3>
