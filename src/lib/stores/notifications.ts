...
export async function markAsRead(id: string) {
  // Optimistic update
  notifications.update((items) => items.map((n) => (n.id === id ? { ...n, is_read: true } : n)));
...
export async function subscribePush() {
  if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
    toast.error(
      get(t)('notifications.toasts.push_not_supported') || 'Push notifications not supported',
    );
    return;
  }
  ...
  try {
    ...
    if (p256dh && auth) {
      ...
      await api.subscribePush(subscription.endpoint, toBase64Url(p256dh), toBase64Url(auth));
      pushEnabled.set(true);
      toast.success(
        get(t)('notifications.toasts.enabled') || 'Push notifications enabled successfully!',
      );
    } else {
