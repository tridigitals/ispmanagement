...
function createUploadStore() {
  ...
  return {
    ...
    upload: async (file: File, token: string) => {
      ...
      const allowedExtsStr =
        settings.storage_allowed_extensions || 'jpg,jpeg,png,gif,pdf,doc,docx,xls,xlsx,zip,mp4,mov';
      ...
      try {
        ...
        while (offset < total) {
          ...
          update((items) => items.map((i) => (i.id === id ? { ...i, progress: percent } : i)));
        }
...
export const isUploading = derived(uploadStore, ($items) =>
