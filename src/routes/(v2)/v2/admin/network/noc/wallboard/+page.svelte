<script lang="ts">
  /*
    Wallboard NOC v2 — gelombang 24d (batch C besar 1/2).

    Versi lama: (app)/admin/network/noc/wallboard/+page.svelte (2105 baris).
    Mode kiosk TV dipertahankan (fullscreen + auto-hide + beep kritis).
    Seluruh logika di modul $lib/components/network/wallboard* dipakai ulang;
    $t() → Indonesia, navigasi keluar → /v2/admin/network/noc.
  */
  ...
  function formatBps(bps?: number | null) {
    return formatWallboardBps(bps, formatBpsValue, '—');
  }

  function formatLatency(ms?: number | null) {
    return formatWallboardLatency(ms, formatLatencyValue, '—');
  }
  ...
  function bucketLabel(bucket: 'raw' | 'hour' | 'day') {
    return bucketLabelValue(bucket, {
      raw: 'Detail',
      hour: 'Rata-rata per jam',
      day: 'Rata-rata harian',
    });
  }
  ...
  function bucketHint(bucket: 'raw' | 'hour' | 'day') {
    return bucketHintValue(bucket, {
      raw: 'tanpa ringkasan',
      hour: 'diringkas per jam',
      day: 'diringkas per hari',
    });
  }
    ...
    <div id="wallboard-alert-panel" class="alert-strip floating-alert-panel">
      {#if WallboardAlertsPanelComponent}
        <WallboardAlertsPanelComponent
          ...
          onOpenAlerts={() => goto('/v2/admin/network/alerts')}
