<script lang="ts">
  ...
  async function saveRole() {
    ...
    try {
      ...
      if (editingRole) {
        await api.roles.update(editingRole.id, roleName, roleDescription, roleLevel, permsArray);
        toast.success(get(t)('admin.roles.toasts.updated') || 'Role updated successfully');
      } else {
        await api.roles.create(roleName, roleDescription, roleLevel, permsArray);
        toast.success(get(t)('admin.roles.toasts.created') || 'Role created successfully');
      }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      toast.success(get(t)('admin.roles.toasts.deleted') || 'Role deleted successfully');
      await loadData();
            ...
            <div class="permissions-container">
              {#each Object.entries(permissionGroups) as [resource, groupPerms]}
                {@const allSelected = groupPerms.every((p) =>
                  selectedPermissions.has(`${p.resource}:${p.action}`),
                )}
                <div class="resource-row">
                  ...
                  <div class="perms-list">
                    {#each groupPerms as perm}
                      ...
                      <label class="perm-toggle {isChecked ? 'checked' : ''}">
                        <input
                          type="checkbox"
                          checked={isChecked}
