<script lang="ts">
  ...
  async function ensureRouterFormModalComponent() {
    if (RouterFormModalComponent) return;

    const modules = await loadRouterFormModal();
    RouterFormModalComponent = modules.RouterFormModalComponent;
  }
  ...
  async function openCreate() {
    ...
    await ensureRouterFormModalComponent();
    showModal = true;
  }

  async function openEdit(r: RouterRow) {
    editing = r;
    formName = r.name || '';
    formHost = r.host || '';
...
{#if RouterFormModalComponent}
  <RouterFormModalComponent
    bind:show={showModal}
    {editing}
    bind:formName
    bind:formHost
