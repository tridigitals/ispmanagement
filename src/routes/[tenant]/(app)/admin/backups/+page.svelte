<script lang="ts">
    ...
    const canRead = $derived(($user?.is_super_admin ?? false) || $user?.role === 'Owner' || $can('read', 'backups'));
    const canCreate = $derived(($user?.is_super_admin ?? false) || $user?.role === 'Owner' || $can('create', 'backups'));
    const canDownload = $derived(($user?.is_super_admin ?? false) || $user?.role === 'Owner' || $can('download', 'backups'));
    const canRestore = $derived(($user?.is_super_admin ?? false) || $user?.role === 'Owner' || $can('restore', 'backups'));
    const canDelete = $derived(($user?.is_super_admin ?? false) || $user?.role === 'Owner' || $can('delete', 'backups'));
...
<div class="tenant-content fade-in">
    <div class="header-section">
        <div>
            ...
            <p class="muted">
                Export and restore your organization data.
            </p>
        </div>
        <div class="flex gap-2 header-actions">
            ...
            <button
                ...
                onclick={() => {
                    // @ts-ignore
                    const isTauri = typeof window !== 'undefined' && !!(window as any).__TAURI_INTERNALS__;
                    if (isTauri) handleRestore(); else fileInput?.click();
                }}
                    ...
                    <tbody>
                        {#each backups as backup (backup.name)}
                            <tr>
                                ...
                                <td class="nowrap">{formatDate(backup.created_at)}</td>
                                <td class="text-right">
