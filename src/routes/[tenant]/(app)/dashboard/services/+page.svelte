<script lang="ts">
  ...
  import { api, type CustomerSubscriptionView } from '$lib/api/client';
  ...
  async function loadData() {
    ...
    try {
      ...
      toast.error(get(t)('dashboard.packages.toasts.load_failed') || 'Failed to load services');
    } finally {
      loading = false;
    }
  }
  ...
  function serviceStatusMeta(
    ...
    if (canRequestReopen) {
      return {
        label: 'Cancelled',
        tone: 'cancelled',
        hint: 'Request was cancelled. You can submit reopen request.',
      };
    }
    ...
    if (s === 'active') {
      return {
        ...
        tone: 'active',
        hint: 'Service is active and running.',
      };
    }
    if (s === 'pending_installation') {
      return {
        ...
        tone: 'pending',
        hint: 'Waiting assignment and installation scheduling.',
      };
    }
    if (s === 'suspended') {
      const awaitingPayment = !startsAt;
      return {
        label: awaitingPayment ? 'Awaiting Payment' : 'Suspended',
        tone: 'suspended',
  ...
  async function requestReopen(sub: CustomerSubscriptionView) {
    ...
    try {
      ...
      toast.error(e?.message || 'Failed to reopen installation request');
    } finally {
      reopeningId = null;
    }
  }
  ...
  function activeFilterLabel(filter: StatusFilter) {
    if (filter === 'all') return 'All Services';
    if (filter === 'active') return 'Active';
    if (filter === 'pending_installation') return 'Pending Installation';
  ...
  function activeSortLabel(key: SortBy) {
    ...
    if (key === 'location_label') return 'Location';
    return 'Service';
  }

  const tableColumns = $derived.by(() => [
    { key: 'package_name', label: 'Service', sortable: true },
    { key: 'status', label: 'Status', sortable: true },
    { key: 'location_label', label: 'Location', sortable: true },
...
<div class="services-content fade-in">
  <section class="page-head">
    <div>
      ...
      <p>List of all services you have ordered.</p>
    </div>
  ...
  <section class="stats-grid">
    ...
    >
      ...
      <div class="stat-content">
        ...
        <span class="stat-label">Pending Installation</span>
      </div>
    </button>
  ...
  <section class="list-panel">
    <div class="panel-head">
      <div>
        ...
        <p>Services requested by your account.</p>
      </div>
      <span class="count-pill">{totalCount} services</span>
    </div>
    ...
    <div class="list-tools">
      ...
      <div class="active-filter-chip">
        <span>Sort: {activeSortLabel(sortBy)} ({sortDirection.toUpperCase()})</span>
      </div>
    </div>
    ...
    {:else if filteredSubscriptions.length === 0}
      <div class="state-block empty">
        ...
        {#if totalCount === 0}
          <p>No ordered services yet.</p>
          <button class="btn primary" type="button" onclick={() => goto('/dashboard/services/order')}>
            <Icon name="plus" size={15} />
            Order New Service
          </button>
        {:else}
          <p>No service found for selected filter.</p>
          <button class="btn ghost" type="button" onclick={clearFilter}>Show All Services</button>
        {/if}
      </div>
    {:else}
              ...
              <div class="table-actions">
                {#if item.can_request_reopen}
                  ...
                  >
                    ...
                    {reopeningId === item.id ? 'Reopening...' : 'Request Reopen'}
                  </button>
                {/if}
          ...
          <article class="service-card">
            ...
            <div class="service-actions">
              {#if sub.can_request_reopen}
                ...
                >
                  <Icon name="rotate-ccw" size={14} />
                  {reopeningId === sub.id ? 'Reopening...' : 'Request Reopen'}
                </button>
              {/if}
