<script lang="ts">
  ...
  onMount(async () => {
    ...
    if ($isAuthenticated) {
      ...
      if (urlTenant && slug && slug.toLowerCase() !== urlTenant.toLowerCase()) {
        ...
        const msg =
          $t('auth.login.error_wrong_tenant') || 'You do not have access to this workspace.';
  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      if (urlTenant && userSlug && urlTenant.toLowerCase() !== userSlug.toLowerCase()) {
        ...
        if (response.user.is_super_admin) {
          ...
          if (isTauri) {
            ...
            throw new Error(`${msg} (Target: ${urlTenant}, Your Account: ${userSlug})`);
          }
        }
      }
      ...
      {:else if step === '2fa-totp'}
        ...
        <div class="twofa-section" in:fade>
          ...
          <button
            class="btn-link"
            on:click={() => {
              step = available2FAMethods.length > 1 ? '2fa-select' : 'login';
              error = '';
            }}
          >
            {$t('common.back') || 'Back'}
