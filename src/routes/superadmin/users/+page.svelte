<script lang="ts">
    ...
    function getTenantName(u: any) {
        ...
        return (
            (id && tenantNameById[id]) || (slug && tenantNameBySlug[slug]) || ""
        );
    }
    ...
    let statusConfirmTitle = $derived.by(() =>
        pendingIsActive
            ...
            : $t("superadmin.users.status.deactivate_title") ||
              "Deactivate User",
    );
    ...
    let statusConfirmMessage = $derived.by(() => {
        ...
        const name =
            u?.name || $t("superadmin.users.status.this_user") || "this user";
        if (pendingIsActive) {
            return (
    ...
    function confirmToggleActive(u: User) {
        if ((u as any).is_super_admin) {
            toast.error(
                get(t)(
                    "superadmin.users.toasts.superadmin_cannot_deactivate",
